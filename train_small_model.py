...
from utils import EVAL_FN, TEST_FN, OPTIMIZER_FN, SCHEDULER_FN
...
def load_data(data_dir: str, used_modals: List[str], model_name: str, tokenizer=None):
    ...
    train_dataset = CMSHDDataset(train_data, used_modals, False, ['sentiment'] if model_name == 'dip' else [])
    valid_dataset = CMSHDDataset(valid_data, used_modals, False, ['sentiment'] if model_name == 'dip' else [])
    test_dataset = CMSHDDataset(test_data, used_modals, False, ['sentiment'] if model_name == 'dip' else [])
...
def build_model(model_name: str, modals_config: dict, model_config: dict):
    if model_name == 'mcwf':
        ...
        model = DIP(modals_config, model_config)
    else:
...
def main():
    config_file_path = 'config/small_model/main.yaml'
    ...
    if len(sys.argv) != 1:
        # task, model, modal
        common_config['task'] = sys.argv[1]
        config['model_name'] = sys.argv[2]
        common_config['used_modals'] = sys.argv[3].split()
        common_config['data_dir'] = build_path('cache', common_config['task'])
    ...
    custom_file_path = f'config/small_model/{model_name}.yaml'
    ...
    Trainer(
        ...
        eval_fn=EVAL_FN[model_name],
        test_dataset=test_dataset,
        test_fn=TEST_FN[model_name],
