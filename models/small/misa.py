...
class MISA(SmallModel):
    def __init__(self, modals_config: dict, model_config: dict, **kwargs):
        ...
        self.fusion = nn.Sequential(
            ...
            nn.Linear(aligned_dim * len(modals_config), model_config['num_class']),
        )
    ...
    def forward(
        ...
        y_tilde = self.fusion(h)  # (bs, 2)

        if self.model_config['num_class'] == 2:
            cls_loss = F.cross_entropy(y_tilde, label)
        else:
            cls_loss = F.binary_cross_entropy_with_logits(y_tilde, label)
        diff_loss = 0
        for modal in self.modals_config:
